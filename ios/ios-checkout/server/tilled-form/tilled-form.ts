export const tilledFormScript = (public_key: string, account_id: string) => `
...
(async function () {
    ...
    tilledFormContainer.addEventListener("submit", async function(event) {
        ...
        if(!payment_intent) return tilledFormContainer.innerHTML = 'Payment failed: No payment intent found.';
        console.log(payment_intent);
        const response = await fetch('/orders/'+ payment_intent.metadata.order_number, {
            method: 'GET',
            headers: {
        ...
        const line_items = order.line_items.map((item) => \`<div class="flex justify-between"><p>\${item.name}</p><p>\${"$" + item.price}</p></div>\`).join('');
        ...
        tilledFormContainer.innerHTML = 
            payment_intent.status === 'succeeded' 
                ...
                <div class="flex flex-col">
                    <div class="flex justify-between">
                        <p>Date paid</p>
                        <p>\${new Date(payment_intent.charges[0].captured_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                    </div>
                    <div class="flex justify-between">
                        <p>Amount paid</p>
                        <p>\${formatter.format(payment_intent.charges[0].amount_captured / 100)}</p>
                    </div>
                    <div class="flex justify-between">
                        <p>Payment method</p>
                        <p>\${payment_intent.payment_method.card.brand[0].toUpperCase() + payment_intent.payment_method.card.brand.slice(1)} \${payment_intent.payment_method.card.last4}</p>
                    </div>
                </div>
            </div>
            <div
            ...
            >
                ...
                <h2 class="text-xl text-center flex justify-between">
                    <div>Total:</div>
                    <div>\${formatter.format(payment_intent.charges[0].amount_captured / 100)}</div>
                </h2>
            </div>
            </div>
            \`
