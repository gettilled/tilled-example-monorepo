...
<body class="bg-tilledBg flex overflow-x-scroll h-full justify-start" id="main">
    ...
    <div class="m-4 flex lg:justify-center h-fit w-full">
        <div class="bg-white rounded-lg shadow-sm shadow-slate-300 px-2 pb-4 w-[72rem] flex flex-col">
            <h1 class="text-3xl font-bold mb-6 my-4 mx-6 text-tilledTableBody">Disputes</h1>
            <table id="transaction-table"
                class="divide-y divide-neutral-100 overflow-hidden mx-6 border-b border-neutral-200">
                <thead>
                    <tr class="bg-slate-50">
                        @foreach (['Dispute Date', 'Evidence Due', 'Reason', 'Status', 'Amount'] as $label)
                            <th
    ...
    <script>
        ...
        function fetchData(offset) {
            if (cachedResults[offset]) {
                ...
                fetch(`/listDisputes?offset=${offset}`, {
                        headers: {
        ...
        function renderData(data) {
            ...
            data.items.forEach((item) => {
                ...
                const formattedDisputeDate = formatDate(item.created_at);
                const evidenceDueDate = formatDate(item.closing_at);
                const formattedAmount = new Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: item.currency,
                }).format(item.amount / 100);
                row.innerHTML = `
                <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${formattedDisputeDate}</td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${evidenceDueDate}</td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${item.reason_description}</td>
