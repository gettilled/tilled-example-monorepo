...
<head>
    ...
    <title>Payment Intents Example</title>
...
<body class="bg-tilledBg flex overflow-x-scroll h-full justify-start" id="main">
    ...
    <div class="m-4 flex lg:justify-center h-fit w-full">
        <div class="bg-white rounded-lg shadow-sm shadow-slate-300 px-2 pb-4 flex flex-col">
            <h1 class="text-3xl font-bold mb-6 my-4 mx-6 text-tilledTableBody">Payment Intents</h1>
            <table id="transaction-table"
                class="divide-y divide-neutral-100 overflow-hidden mx-6 border-b border-neutral-200">
                <thead>
                    <tr class="bg-slate-50">
                        @foreach (['Transaction Date', 'Payment Method', 'Card Holder', 'Number', 'Currency', 'Status', 'Amount'] as $label)
                            <th
    ...
    <script>
        ...
        function fetchData(offset) {
            if (cachedResults[offset]) {
                ...
                fetch(`/listPaymentIntents?offset=${offset}`, {
                        headers: {
        ...
        function renderData(data) {
            ...
            data.items.forEach((item) => {
                ...
                const formattedCreatedDate = formatDate(item.created_at);
                const formattedAmount = new Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: item.currency,
                }).format(item.amount / 100);
                const cardOrBank = (item) => {
                    if (item.payment_method?.type === "card") {
                        return item.payment_method?.card.last4;
                    } else if (item.payment_method?.type === "ach_debit") {
                        return "**" + item.payment_method?.ach_debit.last2;
                    } else {
                        return "-";
                    }
                };
                const isRefunded = (item) => {
                    if (item.charges[0]?.amount_refunded === item.amount && item.charges[0]?.refunded ===
                        true) {
                        return "refunded";
                    } else if (item.charges[0]?.amount_refunded > 0 && item.charges[0]?.amount_refunded < item
                        .amount) {
                ...
                <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${
                    item.payment_method?.type ? (item.payment_method.type[0].toUpperCase() + item.payment_method.type.slice(1)).replace("Ach_d", "ACH D").replace("Eft_d", "EFT D") : "-"
                }
                    </td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">
                ${item.payment_method?.billing_details?.name || "-"}
                </td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${cardOrBank(item)}</td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${item.currency.toUpperCase()}</td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">
              ${isRefunded(item)
                  .split("_")
