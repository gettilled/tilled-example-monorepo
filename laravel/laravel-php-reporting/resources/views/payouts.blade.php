...
<body class="bg-tilledBg flex overflow-x-scroll h-full justify-start" id="main">
    ...
    <div class="m-4 flex lg:justify-center h-fit w-full">
        <div class="bg-white rounded-lg shadow-sm shadow-slate-300 px-2 pb-4 w-[72rem] flex flex-col">
            <h1 class="text-3xl font-bold mb-6 my-4 mx-6 text-tilledTableBody">Payouts</h1>
            <table id="transaction-table"
                class="divide-y divide-neutral-100 overflow-hidden mx-6 border-b border-neutral-200">
                <thead>
                    <tr class="bg-slate-50">
                        @foreach (['Payout Date', 'Payout ID', 'Status', 'Transaction Count', 'Net Amount'] as $label)
                            <th
    ...
    <script>
        ...
        function fetchData(offset) {
            if (cachedResults[offset]) {
                ...
                fetch(`/listPayouts?offset=${offset}`, {
                        headers: {
        ...
        function renderData(data) {
            ...
            data.items.forEach((item) => {
                // Changes the status to withdrawn if the transaction has 0 transactions or if the amount is negative
                if (item.transaction_count === 0 || (item.balance_transaction && item.amount < 0)) {
                ...
                const formattedPayoutDate = formatDate(item.paid_at);
                const formattedAmount = new Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: item.currency,
                }).format(item.amount / 100);
                row.innerHTML = `
                <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${formattedPayoutDate}</td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${item.id}</td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">
              ${item.status[0].toUpperCase() + item.status.slice(1)}
              </td>
            <td class="px-6 py-3 text-xs text-tilledTableBody whitespace-nowrap">${item.transaction_count}</td>
