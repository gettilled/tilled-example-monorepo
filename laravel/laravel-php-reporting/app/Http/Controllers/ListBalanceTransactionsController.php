...
class ListBalanceTransactionsController extends Controller
{
    public function listBalanceTransactions(Request $request)
    {
        ...
        try {
            ...
            $response = Http::withHeaders($headers)->timeout(10)->get($baseUrl . '/v1/balance-transactions?offset= ' . $offset . ' &payout_id=' . $payoutId . '&limit=' . $limit);
            // Check if the API request was successful
            if ($response->successful()) {
                $data = $response->json();
                // Return the list of Balance Transactions as a JSON response
