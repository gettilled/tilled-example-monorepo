...
class ListPayoutsController extends Controller
{
    public function ListPayouts(Request $request)
    {
        ...
        try {
            ...
            $response = Http::withHeaders($headers)->timeout(10)->get($baseUrl . '/v1/payouts?offset= ' . $offset . '&include=transaction_count' . '&limit=15');
            // Check if the API request was successful
            if ($response->successful()) {
                $data = $response->json();
                // Return the list of Payouts as a JSON response
