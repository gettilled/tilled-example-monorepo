...
class ListDisputesController extends Controller
{
    public function ListDisputes(Request $request)
    {
        ...
        try {
            ...
            $response = Http::withHeaders($headers)->timeout(10)->get($baseUrl . '/v1/disputes?offset= ' . $offset . '&limit=15');
            // Check if the API request was successful
            if ($response->successful()) {
                $data = $response->json();
                // Return the list of Disputes as a JSON response
